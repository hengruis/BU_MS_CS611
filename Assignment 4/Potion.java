...
public class Potion extends Item implements Consumable {
    ...
    public void used(Hero hero) {
        ...
        System.out.println(LMaH.ANSI_YELLOW + hero.name + " used " + name + "." + LMaH.ANSI_RESET);
        hero.equips.remove(this);
        System.out.printf(LMaH.ANSI_GREEN + """
                HP\t+%d
                Mana\t+%d
                Strength\t+%d
                Dexterity\t+%d
                Defense\t+%d
                Agility\t+%d
                """ + LMaH.ANSI_RESET, hpRecover, manaRecover, effects[2], effects[3], effects[4], effects[5]);
    }

    @Override
    public String toString() {
        return "(Potion) " + name + ", price: " + price + ", reqLV: " + reqLV + " effects: " + Arrays.toString(effects) +
                "\n";
    }
}
